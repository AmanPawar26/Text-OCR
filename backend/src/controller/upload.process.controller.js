...
export const processInstantOCR = async (req, res) => {
  try {
    ...
    const { csvPath, csvRow } = await appendToCsv(req.file.filename, extractedText);
  ...
  text: extractedText,                      // frontend: results.text
  preview: extractedText.slice(0, 200),     // frontend: results.preview
  csv: csvRow,                              // frontend: results.csv
